                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            ...
                            $values = $article->getArticle($conn);
                            // print_r($values);
                            // var_dump(explode(",",$values[0]['tags']));
                            $tagies = explode(",",$values[0]['tags']);
                            ?>
                        </div>
                        <form method="POST" action="controller-article.php">
                        <input type="hidden" name="id" id="name" value=<?php echo $_GET['id']; ?> >
                            <label for="name">Titre :</label>
                            <input type="text" name="article_name" id="name" value="<?php echo $values[0]['title']; ?>" required>
                            <!-- <label for="name">slug :</label>
                            <input type="text" name="article_slug" id="name" required> -->
                            <label for="description">Content: </label>
                            <textarea name="description_article" id="description" value="<?php echo $values[0]['content']; ?>" required><?php echo $values[0]['content']; ?></textarea>
                            ...
                            <select name="category_name" class="form-control input-solid" id="categorieSelect" name="categorieSelect">
                                <?php foreach ($categories as $categorie): ?>
                                    <option value="<?= htmlspecialchars($categorie['id']); ?>" 
                                        <?php if ($categorie['nom_category'] == $values[0]['category_name']) echo 'selected'; ?>>
                                        <?= htmlspecialchars($categorie['nom_category']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <label for="name">Tags :</label>
                            <?php foreach ($tags as $index => $tag): ?>
                                <input type="checkbox" id="tag<?= $tag['id'] ?>" name="tags[]" value="<?= htmlspecialchars($tag['id']) ?>" 
                                <?php if (in_array($tag['nom_tag'], $tagies)) echo 'checked'; ?>>
                                <label for="tag<?= $tag['id'] ?>"><?= htmlspecialchars($tag['nom_tag']) ?></label><br>
                            <?php endforeach; ?>

                            </br><label for="name">meta description :</label>
                            <input type="text" name="article_meta_description" id="name" value="<?php echo $values[0]['meta_description']; ?>" required>
